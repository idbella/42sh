...
/*   Updated: 2019/12/18 10:37:59 by sid-bell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_getprompt()
{
	...
	if ((path = ft_getpwd()))
	{
		prompt = ft_join("[\e[90m%s\e[0m][%s%d\e[0m] \e[36m%s $> \e[0m", tmp, clr,get_shell_cfg(0)->id++, path);
		free(path);
	}
	else
		prompt = ft_join("[\e[90m%s\e[0m][%s%d\e[0m] $> \e[0m", tmp, clr, get_shell_cfg(0)->id++);
	free(tmp);
	return (prompt);
}
